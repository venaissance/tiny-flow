...
- :test_tube: **297 Tests** -- 257 backend (pytest) + 30 frontend (vitest) + 10 E2E (Playwright)
...
<img src="docs/assets/tinyflow-e2e-flash.png" alt="Flash Mode" width="400" /><br/>
<strong>Flash</strong> -- Direct answer, minimal latency
</td>
<td align="center" width="50%">
<img src="docs/assets/tinyflow-e2e-thinking.png" alt="Thinking Mode" width="400" /><br/>
<strong>Thinking</strong> -- Deep reasoning with context compaction
</td>
</tr>
<tr>
<td align="center" width="50%">
<img src="docs/assets/tinyflow-e2e-pro.png" alt="Pro Mode" width="400" /><br/>
<strong>Pro</strong> -- Plan, execute, and generate artifacts
</td>
<td align="center" width="50%">
<img src="docs/assets/tinyflow-e2e-ultra.png" alt="Ultra Mode" width="400" /><br/>
<strong>Ultra</strong> -- Parallel sub-task research and merge
</td>
</tr>
</table>
...
cp .env.example .env
# Edit .env — at minimum, set one of: GLM_API_KEY, OPENAI_API_KEY, ANTHROPIC_API_KEY
...
│   │   │   └── nodes/          # router, plan, execute, merge, reflect, respond
│   │   ├── executor/           # Parallel task executor pool
│   │   ├── memory/             # Fact extraction → scoring → merging → injection
│   │   ├── middleware/         # Loop detection, TODO tracking, context compaction
│   │   ├── models/             # Multi-provider LLM factory (OpenAI, Claude, GLM)
...
```yaml
model:
  default: "glm-4-flash-250414"    # Default LLM model
...
```bash
# Backend unit & integration tests (257 tests)
cd backend
make test
...
# E2E tests (10 tests, requires both servers running)
cd frontend
pnpm test:e2e
```
...
|----------|-----------|---------|
| LLM Orchestration | LangGraph, LangChain | State machine, model abstraction |
...
- :test_tube: **297 个测试** -- 257 后端（pytest）+ 30 前端（vitest）+ 10 端到端（Playwright）
...
<tr>
<td align="center" width="50%">
<img src="docs/assets/tinyflow-e2e-flash.png" alt="Flash 模式" width="400" /><br/>
<strong>Flash</strong> -- 直接回答，极低延迟
</td>
<td align="center" width="50%">
<img src="docs/assets/tinyflow-e2e-thinking.png" alt="Thinking 模式" width="400" /><br/>
<strong>Thinking</strong> -- 深度推理，上下文压缩
</td>
</tr>
<tr>
<td align="center" width="50%">
<img src="docs/assets/tinyflow-e2e-pro.png" alt="Pro 模式" width="400" /><br/>
<strong>Pro</strong> -- 任务规划、分步执行、产物生成
</td>
<td align="center" width="50%">
<img src="docs/assets/tinyflow-e2e-ultra.png" alt="Ultra 模式" width="400" /><br/>
<strong>Ultra</strong> -- 子任务并行研究与结果合并
</td>
</tr>
</table>
...
- 至少一个 LLM API Key（GLM、OpenAI 或 Anthropic）
...
# 复制环境变量文件，填入 API Key
cp .env.example .env
# 编辑 .env，至少设置一个：GLM_API_KEY、OPENAI_API_KEY、ANTHROPIC_API_KEY
...
│   │   ├── models/             # 多 Provider LLM 工厂（OpenAI、Claude、GLM）
│   │   ├── skills/             # YAML 技能注册中心和路由器
│   │   └── tools/              # 网页搜索等工具
│   ├── skills/                 # 内置技能定义（YAML frontmatter）
│   ├── tests/                  # 257 个 pytest 测试
...
```yaml
model:
  default: "glm-4-flash-250414"    # 默认 LLM 模型
...
| 类别 | 技术 | 用途 |
|------|------|------|
| LLM 编排 | LangGraph, LangChain | 状态机、模型抽象层 |
