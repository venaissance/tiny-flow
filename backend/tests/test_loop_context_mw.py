...
class TestSmartContextCompaction:
    ...
    def test_smart_silent_compaction_no_summary_in_messages(self):
        """Summary stored in metadata only — no synthetic messages injected.
        Node functions read it from metadata for system prompt injection."""
        msgs = [HumanMessage(content=f"m{i}") for i in range(30)]
        mw = ContextCompactionMiddleware(
            max_messages=10,
            strategy="smart",
        ...
        # No summary content in any message
        for m in result["messages"]:
            assert "SUMMARY" not in str(m.content)
            assert "总结" not in str(m.content)
